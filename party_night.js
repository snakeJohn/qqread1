...
!(async () => {
     ...
     for (let i = 0; i < cookiesArr.length; i++) {
          ...
          for (let j = 0; j < $.inviteCodeList.length && $.canHelp; j++) {
               ...
               await takePostRequest('party1031_assist');
          }
          //await $.wait(3000);
     }
})()
...
async function partyNight(){
     $.mainInfo = {};
     await takePostRequest('party1031_init');
     ...
     //预约
     await $.wait(2000);
     await takePostRequest('party1031_remind');
...
async function takePostRequest(type) {
     ...
     switch (type) {
          case 'party1031_init':
               body = `functionId=party1031_init&body={}&client=wh5&clientVersion=1.0.0&appid=o2_act`;
               myRequest = getPostRequest(`party1031_init`, body);
               break;
          case 'party1031_remind':
               body = `functionId=party1031_remind&body={}&client=wh5&clientVersion=1.0.0&appid=o2_act`;
               myRequest = getPostRequest(`party1031_remind`, body);
               break;
          case 'party1031_assist':
               body = `functionId=party1031_assist&body={"inviteCode":"${$.inviteCode}"}&client=wh5&clientVersion=1.0.0&appid=o2_act`;
               myRequest = getPostRequest(`party1031_assist`, body);
               break;
          default:
               console.log(`错误${type}`);
     }
